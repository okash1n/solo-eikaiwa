...
pub fn verify_artifact(
  ...
  let artifact = std::fs::read(artifact_path)
    .map_err(|_| "updater artifactを読み取れません".to_owned())?;
...
pub fn verify_bytes(
  ...
  let signature = Signature::decode(&signature_text)
    .map_err(|_| "updater署名の形式が不正です".to_owned())?;
...
mod tests {
  ...
  fn 生成物が変われば署名検証を拒否する() {
    ...
    assert_eq!(result, Err("updater署名が公開鍵または生成物と一致しません".to_owned()));
  }

  #[test]
  fn 二重base64ではない公開鍵を拒否する() {
    let result = verify_bytes(b"test", &encoded(SIGNATURE), "not-base64");
    assert_eq!(result, Err("updater公開鍵がbase64形式ではありません".to_owned()));
  }
}
