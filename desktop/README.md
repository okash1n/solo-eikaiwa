...
の既存経路を通るため、旧 sidecar は確実に終了してから新バージョンが起動する（現行 Cargo.lock の
tauri 2.11.5 / tauri-plugin-updater 2.10.1。tauri 2.11.5 ソースで、非メインスレッドからの
`restart()` が `RunEvent::Exit` を経由することを確認済み）。適用失敗（App Translocation = `/Applications` 未移動が典型）は Releases への
