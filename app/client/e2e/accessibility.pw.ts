...
// #229 拡張4: 学習ガイドは深いリンクで開け、見出し構造と外部リンクの属性が支援技術に伝わる
test("学習ガイドの見出し構造とREADMEリンクが支援技術に伝わる", async ({ page }) => {
  await page.goto("/#/guide");
  await expect(page).toHaveTitle("Learning Guide — solo-eikaiwa");
  await expect(page.getByRole("heading", { name: "Learning Guide", level: 2 })).toBeVisible();
  for (const section of ["A simple shape for a day", "What each menu is for", "Ideas by level band", "Why this order"]) {
    await expect(page.getByRole("heading", { name: section, level: 3 })).toBeVisible();
  }
  for (const stage of ["Listen", "Memorize", "Speak"]) {
    await expect(page.getByRole("heading", { name: stage, level: 4, exact: true })).toBeVisible();
  }
  const readme = page.getByRole("link", { name: /Why this design/ });
  await expect(readme).toHaveAttribute("href", "https://github.com/btajp/solo-eikaiwa#学習設計の根拠");
  await expect(readme).toHaveAttribute("target", "_blank");
  await expect(readme).toHaveAttribute("rel", "noopener noreferrer");
  await expect(page.getByRole("navigation", { name: "Main navigation" }).getByRole("button", { name: "Learning Guide", exact: true })).toHaveAttribute("aria-current", "page");
  await expectNoSeriousViolations(page);
});
...
test("狭幅でもすべての主ナビゲーションをキーボードで操作できる", async ({ page }) => {
  ...
  const navigationLabels = ["Home", "Learning Guide", "Free Talk", "390 Sentences", "Listening", "Level Check", "Model Talks", "Progress", "Practice reactions", "Settings"];
