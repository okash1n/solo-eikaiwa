...
export type CoachRoutesDeps = {
  aeFeedback: (args: { transcript: string; topicTitle: string; signal?: AbortSignal }) => Promise<AeFeedback>;
  /** 未知の topicId は null（ルートは404を返す）。題名はライブラリ記録用（レスポンスには含めない） */
  modelTalk: (topicId: string, signal?: AbortSignal) => Promise<{ text: string; topicTitle?: string; topicTitleJa?: string } | null>;
  /** モデルトークの記録と一覧（実体は db.ts、テストはフェイク/インメモリ） */
  libraryStore: LibraryStore;
  reflection: (sessionId: string, signal?: AbortSignal) => Promise<Reflection>;
  /** 未知の topicId は null（ルートは404を返す） */
  prepPack: (topicId: string, signal?: AbortSignal) => Promise<PrepPack | null>;
  /** モデルトークの日本語訳＋表現解説を生成（実体は coach.ts、テストはフェイク） */
  explainTalk: (text: string, signal?: AbortSignal) => Promise<{ text: string }>;
  /** モデルトーク解説のキャッシュ（実体は db.ts、テストはフェイク） */
  talkExplainCache: TalkExplainCache;
  /** AI発話の日本語訳のみを生成（実体は coach.ts、テストはフェイク） */
  translate: (text: string, signal?: AbortSignal) => Promise<{ text: string }>;
  ...
  phraseHint: (args: { jaText: string; history?: Array<{ role: "you" | "ai"; text: string }>; signal?: AbortSignal }) => Promise<{ suggestions: Array<{ en: string; ja: string }> }>;
  /** 訂正（original→better）の詳しい日本語解説を生成（実体は coach.ts、テストはフェイク・キャッシュしない） */
  fixExplain: (args: { original: string; better: string; note?: string; signal?: AbortSignal }) => Promise<{ text: string }>;
...
async function handleAeFeedback(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  const fb = await deps.aeFeedback({ transcript, topicTitle: topicTitle ?? "", signal: req.signal });
...
async function handleReflection(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  const refl = await deps.reflection(sessionId, req.signal);
...
async function handleModelTalk(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  const talk = await deps.modelTalk(topicId, req.signal);
...
async function handlePrep(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  if (typeof topicId !== "string" || !topicId.trim()) return json({ error: "topicId is required" }, 400);
  if (topicId.length > 200) return json({ error: "topicId must be at most 200 characters" }, 400);
  const pack = await deps.prepPack(topicId, req.signal);
...
async function respondHashCached(
  ...
  generate: (text: string, signal?: AbortSignal) => Promise<{ text: string }>,
  ...
  const generated = await generate(text, req.signal);
...
async function handlePhraseHint(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  const safeHistory = Array.isArray(history)
    ...
    : undefined;
  const result = await deps.phraseHint({ jaText, history: safeHistory, signal: req.signal });
  return json(result);
}
...
async function handleFixExplain(req: Request, deps: CoachRoutesDeps): Promise<Response> {
  ...
  const result = await deps.fixExplain({ original, better, note: safeNote, signal: req.signal });
  return json(result);
}
