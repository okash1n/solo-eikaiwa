...
- **シャドーイング（5分）** — AI が生成した短いモデルトークに重ねて発話。再生中は停止でき、再生に失敗してもキャッシュ済み音声をその場で再試行できる。「日本語訳と解説」ボタンで全文訳と表現ポイントも確認できる
...
会話でそのまま使える文法・言い回しを網羅したオリジナル390文。文法・機能25分類（日常/ビジネス/IT）に加えて、依頼・断り・聞き返し・言い換え・相槌という5つの会話機能を、難易度帯（入門/中級/上級）ごとに30文ずつ収録しています。**日本語を見て声に出す → 答え合わせ（音声つき）→ 3段階の自己評価**という産出リトリーバル型で、評価に応じて 1→3→7→14→30→60日 の間隔反復（SRS）が次の出題日を決めます。「1日の新規」は練習タブを開き直すか「この件数で練習を読み直す」を押すと新しいキューに反映され、初回採点数を日単位で数えるため設定値を超えて増えません。通信再試行が重なっても、同じ回答のSRS更新とXP付与は1回だけ記録されます。一覧モードでは英文・日本語・番号・カテゴリで検索し、40件ずつ音声つきで見返せます。4/3/2と振り返りで直した表現は、実際に新規保存できたものだけをその場で確認し、マイフレーズ一覧へ移動できます。不要なものはデータを消さずに非表示にでき、非表示中は復習キューから外れ、非表示一覧からいつでも復元できます。
...
GitHub Releases から dmg をダウンロードして起動するだけで動く、ネイティブウィンドウ版のアプリです（Apple Silicon Mac 専用）。サーバ本体・教材コンテンツ・音声認識エンジンをアプリに同梱しているため、Bun や Homebrew のセットアップは不要です（初回のみ音声認識モデルのダウンロードが必要）。機能・データはブラウザ版と共通です。**新しいバージョンは起動時に自動で検知され、ダイアログの「更新する」から最新版を準備できます**（v0.29.0〜・強制更新はしません）。確認中・ダウンロード中はアプリメニューで状態を確認でき、適用後は再起動するタイミングを選べます。入手方法・初回セットアップの詳しい説明は後述「起動: デスクトップアプリ（Tauri）」を参照してください。
...
- **初回のみ課金・以後は `data/tts-cache` からローカル再生で無料**: 音読ウォームアップ・4/3/2 の準備フレーズ（準備フレーズの音声は同梱対象外で、再生時にその場で合成される設計。上の「教材ラダー」節に既述）。モデルトーク一覧から過去のモデルトークを再生する場合も同じ扱い
