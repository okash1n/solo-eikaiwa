...
会話でそのまま使える文法・言い回しを網羅したオリジナル390文。文法・機能25分類（日常/ビジネス/IT）に加えて、依頼・断り・聞き返し・言い換え・相槌という5つの会話機能を、難易度帯（入門/中級/上級）ごとに30文ずつ収録しています。**日本語を見て声に出す → 答え合わせ（音声つき）→ 3段階の自己評価**という産出リトリーバル型で、評価に応じて 1→3→7→14→30→60日 の間隔反復（SRS）が次の出題日を決めます。「1日の新規」は練習タブを開き直しても初回採点数を日単位で数え、設定値を超えて増えません。通信再試行が重なっても、同じ回答のSRS更新とXP付与は1回だけ記録されます。一覧モードでは全文をフィルタ・音声つきで眺められます。

390文すべての音声はリポジトリに同梱済み（`content/sentences/audio/`、OpenAI TTS で事前生成したAI音声）なので、**OpenAI キーなしでもネイティブ品質の音声で練習できます**。

### 🎧 リスニング（多聴）

レベルに合った短い英語を、スクリプトを隠したまま通しで聴くリスニング練習。3つのレベル帯（入門/中級/上級）× 3ドメイン（日常/ビジネス/IT）で均等に用意した42本（各2〜4分）を収録し、音声はすべてリポジトリに同梱済み（OpenAI TTS で事前生成）。音声の準備中でも停止や画面移動が確実に反映され、あとから古い音声が鳴り始めることはありません。聴き終えたらボタンでスクリプト表示・日本語訳と表現解説も確認できます。聴いた本数は「今週◯本」の情報表示のみでノルマはありません。素材は生成CLIで自分のレベルに合わせてさらに増やせます。
...
| OpenAI API | ローカル（OpenAI 互換） | Base URL `https://api.openai.com/v1` + モデル名。接続先を保存後、設定画面でキーを保存してoriginを承認 |
| GitHub Models | ローカル（OpenAI 互換） | Base URL `https://models.github.ai/inference` + モデル名（レート制限に注意）。接続先を保存後、設定画面でPATを保存してoriginを承認 |
