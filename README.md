...
API キーは**アプリ起動後に ⚙️ 設定 → モデル接続設定の各セクションから登録**できます（macOS Keychain に保存・マスク入力・値は UI にもサーバ応答にも出ません）。従来どおり `app/.env` に置くことも可能です（両方ある場合は Keychain が優先。UI には「Keychain / app/.env から検出」とソースが表示されます）:
