...
- **くり返しトーク 4/3/2（8分）** — 同じ話を制限時間を縮めながら話す流暢性トレーニング。準備・発話の時間は同じ `mm:ss` 表記で示し、録音を始めると発話時間が動きます。制限時間に達すると録音を自動停止して文字起こしし、実際に話した時間を記録します。話す前に「表現・話の骨組み・モデルトーク」の準備フェーズがあり、モデルトークの再生中も停止できます。全ラウンドとフィードバックを終えてから次のブロックへ進みます
...
各暦月に1回、Progress 画面で**月次レビュー**も書いてもらえます。直近30日の練習時間・調音速度・例文の定着・収集フレーズなどをまとめた日本語の振り返りレポートです（情報表示のみ・ノルマや判定はありません）。
...
- **練習フィードバック**: セッション完了・自由会話・リスニングの後に「今のはどうでしたか？（難しすぎた/ちょうどよかった/簡単すぎた + 任意メモ）」の1タップ評価。溜まった記録は「フィードバック」画面で一覧・Markdown コピーでき、アプリ改善の入力に使えます（タップしなければ何も起きません）
...
- ホームのドリルは種類ごとの色と、タイトル・説明・所要時間の階層で選びやすく表示する。カード、カード見出し、外部リンクを含むボタンは全画面で同じ部品と文字サイズ設定に従う
