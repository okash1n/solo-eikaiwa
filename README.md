...
- **くり返しトーク 4/3/2（8分）** — 同じ話を制限時間を縮めながら話す流暢性トレーニング。準備・発話の時間は同じ `mm:ss` 表記で示し、録音を始めると発話時間が動きます。制限時間に達すると録音を自動停止して文字起こしし、実際に話した時間を記録します。話す前に「表現・話の骨組み・モデルトーク」の準備フェーズがあり、モデルトークの再生中も停止できます。全ラウンドとコーチからのヒントを確認してから次のブロックへ進みます
...
- サイドバーは**今日の練習 / 自主練 / 記録・測定・設定**の3セクション構成。自主練の取り組み順ヒント（聞く→覚える→話す）を ⓘ で確認できます。「記録・測定・設定」の **⚙️ 設定**で、用途別の LLM、音声エンジン、文字サイズ・言語を変更できます。APIキーは専用タブへ集約され、OpenAI公式とOpenAI互換の独自接続先を混同せず設定できます。
