...
- **音読ウォームアップ（6分）** — 今日のお題の表現チャンクを声に出して準備。各表現のネイティブ音声を聞け、再生中は同じ位置の停止操作で中断できる
...
会話でそのまま使える文法・言い回しを網羅したオリジナル390文。文法・機能25分類（日常/ビジネス/IT）に加えて、依頼・断り・聞き返し・言い換え・相槌という5つの会話機能を、難易度帯（入門/中級/上級）ごとに30文ずつ収録しています。**日本語を見て声に出す → 答え合わせ（音声つき）→ 3段階の自己評価**という産出リトリーバル型で、評価に応じて 1→3→7→14→30→60日 の間隔反復（SRS）が次の出題日を決めます。「1日の新規」は練習タブを開き直すか「この件数で練習を読み直す」を押すと新しいキューに反映され、初回採点数を日単位で数えるため設定値を超えて増えません。通信再試行が重なっても、同じ回答のSRS更新とXP付与は1回だけ記録されます。一覧モードでは全文をフィルタ・音声つきで眺められます。セッションから自動収集されたマイチャンクも同じ画面で確認でき、不要なものはデータを消さずに非表示にできます。非表示中は復習キューから外れ、非表示一覧からいつでも復元できます。
...
- **練習フィードバック**: セッション完了・自由会話・多聴の後に「今のはどうでしたか？（難しすぎた/ちょうどよかった/簡単すぎた + 任意メモ）」の1タップ評価。溜まった記録は「フィードバック」画面で一覧・Markdown コピーでき、アプリ改善の入力に使えます（タップしなければ何も起きません）
