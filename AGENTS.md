...
- **i18n は named 型辞書（`src/i18n.ts`）**: 型 + `STR.en` + `STR.ja` の3点を同時に追加。**既存キーの日本語文言は一字一句変更しない**（変更は明示の合意があるときのみ）。文字列の直書き禁止。
...
- API キー等の secrets は `app/.env` のみ。DB・API レスポンス・ログ・plist に出さない。
